...
class PreferencesPage(ctk.CTkFrame):
    def __init__(self, master, on_save, on_back, **kwargs):
        ...
        # Flight Type
        type_label = ctk.CTkLabel(card, text="Flight Type", font=("Segoe UI", 16, "bold"), text_color="#232946")
        type_label.grid(row=3, column=0, pady=(0, 6), padx=48, sticky="w")
        self.type_dropdown = ctk.CTkOptionMenu(card, values=["All", "Commercial", "Private", "Military"], fg_color="#e0e3eb", button_color="#3f8efc", corner_radius=18, font=("Segoe UI", 15), text_color="#232946", width=0)
        self.type_dropdown.grid(row=4, column=0, padx=48, pady=(0, 16), sticky="ew")
        self.type_dropdown.set("All")
        # Minimum Altitude
        alt_label = ctk.CTkLabel(card, text="Minimum Altitude (m)", font=("Segoe UI", 16, "bold"), text_color="#232946")
        alt_label.grid(row=5, column=0, pady=(0, 0), padx=48, sticky="w")
        self.alt_var = ctk.StringVar(value="0")
        def validate_numeric(new_value):
            return new_value.isdigit() or new_value == ""
        vcmd = (self.register(validate_numeric), '%P')
        alt_entry = ctk.CTkEntry(card, textvariable=self.alt_var, placeholder_text="e.g. 1000", font=("Segoe UI", 15), fg_color="#f5f6fa", border_color="#e0e0e0", border_width=2, corner_radius=18, text_color="#232946", width=0, validate="key", validatecommand=vcmd)
        alt_entry.grid(row=6, column=0, padx=48, pady=(0, 0), sticky="ew")
